<?php
	...
	*/
	...
	if($loUnidad->fbListar_Excel($_POST) === true)
	{
		$laMatriz = $loUnidad->faGetVariables();
	}
	else
	{
	...
	$loXls->getProperties()
			...
			->setSubject("Listado de Unidades")
			->setDescription("Documento generado con PHPExcel")
			->setKeywords("Listado de Unidades")
			->setCategory("Reportes");
	
	$liA = 0;
		
	//Unión de celdas
	//$loXls->setActiveSheetIndex($liA)->mergeCells('A1:C1');
	...
	while($laMatriz[$liE]['liCodigo'] != "")
	{
		...
		//Bordes
		$loXls->getActiveSheet()->getStyle('A'.$liLinea_Actual.':C'.$liLinea_Actual)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
	...
	$lsTitulo_Hoja = 'Listado de Unidades';
	...
	$loXls->getActiveSheet()->getColumnDimension('B')->setWidth(30);
	...
	header('Content-Disposition: attachment;filename="Listado-de-unidades.xls"');
