...
var gsUrl	= "../../controladores/corCategoria.php";
window.onload = () => {
	fpListar();
};
...
function fpNuevo()
{
	document.querySelector('#h4Encabezado_Formulario').innerHTML='Registrar Categoria';
...
function fpBuscar(piCodigo)
{
	if(piCodigo >= 0)
	{
		...
		fpEnviar(loFormulario, laResultado => {
			...
			{
				document.querySelector('#h4Encabezado_Formulario').innerHTML='Actualizar Categoria';
...
function fbValidar()
{
	...
	{
		fpMostrar_Mensaje("Disculpe, el nombre del categoría no puede estar en blanco",'a');
	}
	else
	{
		lbBueno = true;
...
/****************************************************************************************************
	...
	código de la categoría y que al momento de buscar lo haga por coincidencia del nombre y no por código
****************************************************************************************************/
function fpVerificar_Categoria()
{
