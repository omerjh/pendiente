...
var gsUrl	= "../../controladores/corMoneda.php";
window.onload = () => {
	fpListar();
};
...
function fpNuevo()
{
	document.querySelector('#h4Encabezado_Formulario').innerHTML='Registrar Moneda';
...
function fpBuscar(piCodigo)
{
	if(piCodigo >= 0)
	{
		...
		fpEnviar(loFormulario, laResultado => {
			...
			{
				document.querySelector('#h4Encabezado_Formulario').innerHTML='Actualizar Moneda';
				
				//asignacion de valores
				loF.txtOperacion.value	= 'modificar';
				loF.txtNombre.value		= laResultado.laDatos.txtNombre;
				loF.txtSimbolo.value	= laResultado.laDatos.txtSimbolo;
...
function fbValidar()
{
	...
	{
		fpMostrar_Mensaje("Disculpe, el nombre de la moneda no puede estar en blanco",'a');
	}
	else
	{
		lbBueno = true;
...
function fpListar(piPagina)
{
	...
	let laDatos = {
		txtOperacion		: 'listar',
		lsTitulos			: 'Código,Nombre,Símbolo,Estado,Opciones',
...
/****************************************************************************************************
	...
	código de la moneda y que al momento de buscar lo haga por coincidencia del nombre y no por código
****************************************************************************************************/
function fpVerificar_Moneda()
{
