...
var gsUrl	= "../../controladores/corAlmacen.php";
window.onload = () => {
	fpListar();
};
...
function fpNuevo()
{
	document.querySelector('#h4Encabezado_Formulario').innerHTML='Registrar Almacén';
...
{	
	...
	loF.txtCodigo.value		= "";
	loF.txtNombre.value		= "";
	loF.radVenta1.checked	= true;
	loF.radVenta2.checked	= false;
	loF.cmbEmpresa.value	= "-";
...
function fpBuscar(piCodigo)
{
	if(piCodigo >= 0)
	{
		...
		fpEnviar(loFormulario, laResultado => {
			...
			{
				document.querySelector('#h4Encabezado_Formulario').innerHTML='Actualizar Almacén';
				
				//asignacion de valores
				loF.txtOperacion.value	= 'modificar';
				loF.txtNombre.value		= laResultado.laDatos.txtNombre;
				loF.radVenta.value		= laResultado.laDatos.radVenta;
				loF.cmbEmpresa.value	= laResultado.laDatos.cmbEmpresa;
...
function fpListar(piPagina)
{
	...
	let laDatos = {
		txtOperacion		: 'listar',
		lsTitulos			: 'Código,Nombre,Para Venta,Empresa,Estado,Opciones',
		lsPorcentajes		: '10%,30%,15%,25%,10%,10%',
