...
var gsUrl	= "../../controladores/corProveedor.php";
window.onload = () => {
	fpListar();
};
...
function fpNuevo()
{
	document.querySelector('#h4Encabezado_Formulario').innerHTML='Registrar Proveedor';
...
function fpBuscar(piCodigo)
{
	if(piCodigo >= 0)
	{
		...
		fpEnviar(loFormulario, laResultado => {
			...
			{
				document.querySelector('#h4Encabezado_Formulario').innerHTML='Actualizar Proveedor';
...
function fbValidar()
{
	...
	{
		fpMostrar_Mensaje("Disculpe, el nombre del proveedor no puede estar en blanco",'a');
	}
	else
	{
		lbBueno = true;
...
****************************************************************************************************/
function fpVerificar_Proveedor()
{
