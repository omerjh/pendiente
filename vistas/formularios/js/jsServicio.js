...
var gsUrl	= "../../controladores/corServicio.php";
window.onload = () => {
	fpListar();
};
...
function fpNuevo()
{
	document.querySelector('#h4Encabezado_Formulario').innerHTML='Registrar Servicio';
...
{	
	...
	loF.txtCodigo.value				= "";
	loF.txtNombre.value				= "";
	loF.txtNombre_Largo.value		= "";
	loF.txtUrl.value				= "";
	loF.radAbrir_Especial1.checked	= false;
	loF.radAbrir_Especial2.checked	= true;
	loF.txtIcono.value				= "";
	loF.txtPosicion.value			= "0";
	loF.cmbModulo.value				= "-";
	loF.cmbEstado.value				= "A";
	document.querySelector('#divDemostracion').classList.toggle('d-none');
	document.querySelector('#spaIcono').className="control-label";
	$('#cmbModulo').trigger('change.select2');
...
function fpBuscar(piCodigo)
{
	if(piCodigo > 0)
	{
		...
		fpEnviar(loFormulario, laResultado => {
			...
			{
				document.querySelector('#h4Encabezado_Formulario').innerHTML='Actualizar Servicio';
				
				//asignacion de valores
				loF.txtOperacion.value		= 'modificar';
				loF.txtNombre.value			= laResultado.laDatos.txtNombre;
				loF.txtNombre_Largo.value	= laResultado.laDatos.txtNombre_Largo;
				loF.txtUrl.value			= laResultado.laDatos.txtUrl;
				loF.txtIcono.value			= laResultado.laDatos.txtIcono;
				loF.txtPosicion.value		= laResultado.laDatos.txtPosicion;
				loF.cmbModulo.value			= laResultado.laDatos.cmbModulo;
				loF.cmbEstado.value			= laResultado.laDatos.cmbEstatus;
				fpMostrar_Icono(loF.txtIcono, 'divDemostracion', 'spaIcono');
				
				$('#cmbModulo').trigger('change.select2');
				if(laResultado.laDatos.radAbrir_Especial == 'S')
				{
					loF.radAbrir_Especial1.checked	= true;
				}
				else
				{
					loF.radAbrir_Especial2.checked	= true;
...
function fbValidar()
{
	...
	{
		fpMostrar_Mensaje("Disculpe, el nombre del módulo no puede estar en blanco",'a');
	}
	else if(loF.txtPosicion.value == "" || Number(loF.txtPosicion.value) < 0)
	{
		fpMostrar_Mensaje("Disculpe, la posición del módulo no puede estar en blanco",'a');
	}
	else if(loF.cmbModulo.value == "")
	{
		fpMostrar_Mensaje("Disculpe, debe seleccionar el módulo",'a');
	}
	else
	{
		lbBueno = true;
...
function fpListar(piPagina)
{
	...
	let laDatos = {
		txtOperacion		: 'listar',
		lsTitulos			: "Cód,Nombre,Módulo,Url,Ico,Pos,Est,Opciones",
		lsTitles			: "Código,Nombre,Módulo,Url,Icono,Posición,Estatus,Opciones",
		lsPorcentajes		: "6%,28%,23%,23%,5%,5%,5%,5%",
		lsAlineacion		: "right,left,left,left,left,right,left,left",
