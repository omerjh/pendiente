...
var gsUrl	= "../../controladores/corCuenta_Bancaria.php";
window.onload = () => {
	fpListar();
};
...
function fpNuevo()
{
	document.querySelector('#h4Encabezado_Formulario').innerHTML='Registrar Cuenta Bancaria';
...
{	
	...
	$('#cmbBanco').trigger('change.select2');
	$('#cmbMoneda').trigger('change.select2');
...
function fpBuscar(piCodigo)
{
	if(piCodigo >= 0)
	{
		...
		fpEnviar(loFormulario, laResultado => {
			...
			{
				document.querySelector('#h4Encabezado_Formulario').innerHTML='Actualizar Cuenta Bancaria';
				
				//asignacion de valores
				loF.txtOperacion.value	= 'modificar';
				loF.txtNombre.value		= laResultado.laDatos.txtNombre;
				loF.txtNumero.value		= laResultado.laDatos.txtNumero;
				loF.txtSaldo.value		= laResultado.laDatos.txtSaldo;
				loF.cmbBanco.value		= laResultado.laDatos.cmbBanco;
				loF.cmbMoneda.value		= laResultado.laDatos.cmbMoneda;
...
function fbValidar()
{
	...
	{
		fpMostrar_Mensaje("Disculpe, el nombre de la cuenta bancaria no puede estar en blanco",'a');
	}
	else
	{
		lbBueno = true;
...
function fpListar(piPagina)
{
	...
	let laDatos = {
		txtOperacion		: 'listar',
		lsTitulos			: 'Código,Nombre,Número,Saldo,Banco,Moneda,Estado,Opciones',
		lsPorcentajes		: '5%,30%,5%,10%,26%,7%,7%,10%',
		lsAlineacion		: 'right,left,left,left,left,left,left,left',
...
/****************************************************************************************************
	...
	código de la cuenta bancaria y que al momento de buscar lo haga por coincidencia del nombre y no por código
****************************************************************************************************/
function fpVerificar_Cuenta_Bancaria()
{
