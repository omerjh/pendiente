...
var gsUrl	= "../../controladores/corTipo_Producto.php";
window.onload = () => {
	fpListar();
};
...
function fpNuevo()
{
	document.querySelector('#h4Encabezado_Formulario').innerHTML='Registrar Tipo de Producto';
...
{	
	...
	loF.txtCodigo.value		= "";
	loF.txtNombre.value		= "";
	loF.radStock1.checked	= true;
	loF.radStock2.checked	= false;
...
function fpBuscar(piCodigo)
{
	if(piCodigo >= 0)
	{
		...
		fpEnviar(loFormulario, laResultado => {
			...
			{
				document.querySelector('#h4Encabezado_Formulario').innerHTML='Actualizar Tipo de Producto';
				
				//asignacion de valores
				loF.txtOperacion.value	= 'modificar';
				loF.txtNombre.value		= laResultado.laDatos.txtNombre;
				loF.cmbEstado.value		= laResultado.laDatos.cmbEstatus;
				if(laResultado.laDatos.radStock == 'S')
				{
					loF.radStock1.checked	= true;
				}
				else
				{
					loF.radStock2.checked	= true;
				}
				setTimeout(() => { loF.txtNombre.focus(); }, 300);
				$("#divFormulario").modal("show");
			}
...
function fbValidar()
{
	...
	{
		fpMostrar_Mensaje("Disculpe, el nombre del tipo de producto no puede estar en blanco",'a');
	}
	else
	{
		lbBueno = true;
...
function fpListar(piPagina)
{
	...
	let laDatos = {
		txtOperacion		: 'listar',
		lsTitulos			: 'Código,Nombre,Maneja Stock,Estado,Opciones',
...
/****************************************************************************************************
	...
	código de la tipo de producto y que al momento de buscar lo haga por coincidencia del nombre y no por código
****************************************************************************************************/
function fpVerificar_Tipo_Producto()
{
