...
function fpNuevo()
{
	document.querySelector('#h4Encabezado_Formulario').innerHTML='Registrar Rol';
...
function fpListar(piPagina)
{
	...
	let laDatos = {
		txtOperacion		: 'listar',
		lsTitulos			: 'Código,Nombre,Estatus,Opciones',
		lsTitles			: 'Código,Nombre,Estatus,Opciones',
...
****************************************************************************************************/
function fpPintar_Servicio(piCodigo)
{
	let laDatos = {
		txtOperacion: 'listar_servicios',
		txtArchivo	: loF.txtArchivo.value
	}
	let laFormulario = faObtener_Datos_Formulario('frmF');
	
	const loFormulario = new FormData();
	Object.keys(laDatos).forEach((key) => {
		loFormulario.append(key, laDatos[key]);
	});
	Object.keys(laFormulario).forEach((key) => {
		loFormulario.append('laFormulario['+key+']', laFormulario[key]);
	});
	
	document.querySelector('#divServicio').innerHTML="<img src='imagenes/cargando.gif' height='48' width='48'>";
	
	fpEnviar(loFormulario, laResultado => {
		if(laResultado.lbEstado == true)
		{
			document.querySelector('#divServicio').innerHTML=laResultado.lsTabla_Servicio;
		}
		else
		{
			fpMostrar_Mensaje(laResultado.lsMensaje, '-');
