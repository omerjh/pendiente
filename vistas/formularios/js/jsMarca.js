...
var gsUrl	= "../../controladores/corMarca.php";
window.onload = () => {
	fpListar();
};
...
function fpNuevo()
{
	document.querySelector('#h4Encabezado_Formulario').innerHTML='Registrar Marca';
...
function fpBuscar(piCodigo)
{
	if(piCodigo >= 0)
	{
		...
		fpEnviar(loFormulario, laResultado => {
			...
			{
				document.querySelector('#h4Encabezado_Formulario').innerHTML='Actualizar Marca';
...
function fbValidar()
{
	...
	{
		fpMostrar_Mensaje("Disculpe, el nombre de la marca no puede estar en blanco",'a');
	}
	else
	{
		lbBueno = true;
...
/****************************************************************************************************
	...
	código de la marca y que al momento de buscar lo haga por coincidencia del nombre y no por código
****************************************************************************************************/
function fpVerificar_Marca()
{
