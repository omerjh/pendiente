<?php
	...
	****************************************************************************************************/
	...
	{
		require_once("../modelos/clsUnidad.php");
		$loUnidad				= new clsUnidad();
		if($loUnidad->fpSetFormulario($_POST) === true)
		{
			switch($_POST["txtOperacion"])
			{
				case "buscar":
					if($loUnidad->fbBuscar() === true)
					{
						$laRespuesta['lbEstado']= true;
						$laRespuesta['laDatos']	= $loUnidad->faGetVariables();
					}
					else
					{
				...
				case "incluir":
					if($loUnidad->fbBuscar_Nombre() === true)
					{
						$laRespuesta["lbEstado"]	= false;
						$laRespuesta['lsMensaje']	= "Disculpe, el nombre del unidad o la abreviatura que desea guardar se encuentra registrado";
					}
					elseif($loUnidad->fbIncluir() === true)
					{
						$laRespuesta["lbEstado"]	= true;
				...
				case "modificar":
					if($loUnidad->fbBuscar_Nombre(true) === true)
					{
						$laRespuesta["lbEstado"]	= false;
						$laRespuesta['lsMensaje']	= "Disculpe, el nombre del unidad que desea guardar se encuentra registrado";
					}
					elseif($loUnidad->fbModificar() === true)
					{
						$laRespuesta['lbEstado']	= true;
				...
				case "listar":
					...
					if($loUnidad->fbListar($_POST["laFormulario"]) === true)
					{
						$laVariables= $loUnidad->faGetVariables();
						...
						$loTabla->fpPaginar($loUnidad->fiNumero_Resultados($_POST["laFormulario"]), $_POST["laFormulario"]['txtFiltro_Pagina']);
				...
				case "autocompletado":
					if($loUnidad->fbListar_Autocompletado($_POST["txtBuscar"]) === true)
					{
						$laRespuesta= $loUnidad->faGetVariables();
					}
					else
					{
					}
