<?php
	...
	****************************************************************************************************/
	...
	else
	{
		...
		if($loModulo->fpSetFormulario($_POST) === true)
		{
			switch($_POST["txtOperacion"])
			{
				case "buscar":
					if($loModulo->fbBuscar() === true)
					{
						$laRespuesta['lbEstado']= true;
						$laRespuesta['laDatos']	= $loModulo->faGetVariables();
					}
					else
					{
				...
				case "modificar":
					if($loModulo->fbBuscar_Nombre(true) === true)
					{
						$laRespuesta["lbEstado"]	= false;
						$laRespuesta['lsMensaje']	= "Disculpe, el nombre del módulo que desea guardar se encuentra registrado";
					}
					elseif($loModulo->fbModificar() === true)
					{
						$laRespuesta['lbEstado']	= true;
				...
				case "listar":
					...
					if($loModulo->fbListar($_POST["laFormulario"]) === true)
					{
						$laVariables= $loModulo->faGetVariables();
						...
						$loTabla->fpPaginar($loModulo->fiNumero_Resultados($_POST["laFormulario"]), $_POST["laFormulario"]['txtFiltro_Pagina']);
				...
				case "autocompletado":
					if($loModulo->fbListar_Autocompletado($_POST["txtBuscar"]) === true)
					{
						$laRespuesta= $loModulo->faGetVariables();
					}
					else
					{
					}
